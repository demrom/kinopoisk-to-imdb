...
didn't really match), or `unmatched` when nothing was found. Rows needing a call
are also written to a focused `<out>.review.json` worklist. Edit `decision`
there (review/unmatched -> accept or reject), then hand the file to `imdb.lists`,
which imports only the `accept` rows. (Pass --format csv only if you'd rather
edit in a spreadsheet — JSON keeps more detail.)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--review-out",
        help="also write only the rows needing a decision here "
             "(default: <out>.review.<ext>); a focused worklist",
    )
    p.add_argument(
        "--no-review-file", action="store_true",
        help="do not write the separate review worklist",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Every row carries an auto-filled `decision`: accept for clean hits, review
    # for anything that looks off (e.g. the title didn't really match), unmatched
    # when nothing was found. Split the ones needing a human call into a focused
    # worklist so you don't scroll the whole file.
    ...
    review_path = None
    if needs and not args.no_review_file:
        if args.review_out:
    ...
    if review_path is not None:
        print(
            f"{len(needs)} need a decision -> {review_path}\n"
            "  edit the 'decision' column (review/unmatched -> accept or reject), "
